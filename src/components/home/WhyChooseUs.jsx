      ...
      <div className="container mx-auto px-6 lg:px-8">
        ...
        >
          {BENEFITS.map((benefit, index) => (
            <motion.div
              key={index}
              className="group p-6 lg:p-8 rounded-2xl bg-[var(--color-bg-card)] border border-[var(--color-border)] hover:border-[var(--color-primary)]/50 transition-all duration-300"
              variants={itemVariants}
              whileHover={{ y: -5, boxShadow: 'var(--shadow-lg)' }}
            >
              {/* Icon */}
              <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-secondary)] flex items-center justify-center text-white mb-5 group-hover:scale-110 transition-transform duration-300">
                {icons[benefit.icon]}
              </div>
